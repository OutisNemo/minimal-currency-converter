...
const debouncedUpdate = debounce(name => update(name), 300);

function onValueChange(event) {
  debouncedUpdate(event.target.name);
}
...
function addCurrency(init) {
  ...
  for (let currency in window.currenciesJSON) {
    ...
    option.title = window.currenciesJSON[currency].title;
...
function convertValue(element, value, from, to) {
  ...
  chrome.runtime.sendMessage(
    { type: "convertValue", value: value, from: from, to: to },
    (response) => {
      if (response.status == "error") {
        ...
        element.value = response.value.toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: precision }) + " " + to;
      }
...
function init() {
  for (
    count_currencies = 0;
    count_currencies < MAX_CURRENCIES;
    count_currencies++
  ) {
